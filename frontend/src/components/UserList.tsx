...
import { Users, User, Crown, UserMinus } from 'lucide-react';
...
const UserList: React.FC<UserListProps> = ({ users, currentUserId, hostId, onKickUser }) => {
        ...
        {users.map((user) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Kick Button (visible only to host, and not shown next to host themself) */}
              {hostId === currentUserId && user.id !== hostId && onKickUser && (
                <button
                  onClick={() => {
                    if (confirm(`Kick ${user.name} from room?`)) onKickUser(user.id);
                  }}
